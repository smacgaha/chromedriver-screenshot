RSpec.describe ChromedriverScreenshot::Tile do
  context '#screenshot' do
    ...
    before(:each) do
      image_blob = ChunkyPNG::Image.from_file(
        'spec/support/fixtures/smiley_face.png'
      ).to_blob
      allow_any_instance_of(described_class).to(
        receive(:screenshot_from_platform).and_return(image_blob)
      )
      allow_any_instance_of(described_class).to(
        receive(:offset_from_platform).and_return(offset)
      )
    end
    ...
    context 'window without offset' do
      ...
      context 'full screen shot' do
        ...
        it "isn't cropped" do
          expected = ChunkyPNG::Image.from_file(
            'spec/support/fixtures/smiley_face.png'
          )
