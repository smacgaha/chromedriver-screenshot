RSpec.describe ChromedriverScreenshot::Page do
  context '#initialize' do
    ...
    before(:example) do
      ...
      allow(ChromedriverScreenshot::Platforms).to(
        receive(:platform).and_return(platform)
      )
      expect(ChromedriverScreenshot::Row).to(
        receive(:from_boundaries).with(expected_ary)
      )
    end
